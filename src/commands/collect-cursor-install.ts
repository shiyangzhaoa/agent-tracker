...
export async function runCollectCursorInstall(cwd: string): Promise<number> {
  ...
  printStatus(
    'ok',
    createdProjectConfig.created ? '已创建项目配置' : '项目配置已存在',
  );
  ...
  printSection('Cursor');
  printStatus('ok', 'agent-tracker 的 Hook 集成已安装或修复');
  ...
  if (cursorStatus.installed) {
    ...
    if (
      cursorStatus.managedEvents.length < installResult.managedEvents.length
    ) {
      printSubItem(
  ...
  printStatus(
    'info',
    '后续只需正常使用 Cursor，Hook 事件会被写入本地 Raw Trace',
  );
  printStatus(
    'info',
    '在 Cursor 编辑后，请打开“健康”页确认 Trace 文件是否开始增长',
  );
