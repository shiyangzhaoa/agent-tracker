...
import { listWorkingTreeFiles, type RepoContext } from "./git";
...
interface CaptureHumanEvidenceOptions {
  hookEventName?: "agentTracker.manualSnapshot" | "agentTracker.reconcileSnapshot";
...
export async function captureHumanEvidenceSnapshots(
  ...
  for (const snapshot of existingSnapshots) {
    const existing = knownContentHashesByPath.get(snapshot.relativePath) ?? new Set<string>();
    existing.add(snapshot.contentHash);
    knownContentHashesByPath.set(snapshot.relativePath, existing);
  }

  const workingTreeFiles = listWorkingTreeFiles(repo.repoRoot);
  let capturedCount = 0;
  let skippedCount = 0;
  ...
  for (const entry of workingTreeFiles) {
    ...
    const snapshot = await captureFileSnapshot(repo.repoRoot, entry.absolutePath, recordedAt);
    ...
    const knownHashes = knownContentHashesByPath.get(snapshot.relativePath) ?? new Set<string>();
